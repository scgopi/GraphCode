...
pub const DaemonClient = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !DaemonClient {
        var client = DaemonClient{ .allocator = allocator };
    ...
    pub fn connect(self: *DaemonClient) void {
        ...
        self.state = if (self.state == .unavailable) .reconnecting else .connecting;
        self.last_error = "";
        ...
        while (attempt < 4) : (attempt += 1) {
            ...
            std.Thread.sleep((50 + attempt * 100) * std.time.ns_per_ms);
        }
        self.state = .unavailable;
        self.last_error = "daemon unavailable; retrying";
    }

    pub fn reconnect(self: *DaemonClient) void {
        if (self.state != .connected) self.connect();
    }
    ...
    pub fn sendCreateNode(self: *DaemonClient, project_path: []const u8, title: []const u8) void {
        const command = Wire.commandGraphCreateNode(self.allocator, project_path, title) catch return;
        defer self.allocator.free(command);
        self.sendCommand(command);
    }
    ...
    pub fn poll(self: *DaemonClient) void {
        if (self.state != .connected or self.pipe == c.INVALID_HANDLE_VALUE) return;
        var available: c.DWORD = 0;
        if (c.PeekNamedPipe(self.pipe, null, 0, null, &available, null) == 0) {
            self.markTransportFailure("daemon connection closed");
            return;
        }
        while (available > 0) {
            const frame = self.receiveFrame() catch {
            ...
            if (c.PeekNamedPipe(self.pipe, null, 0, null, &available, null) == 0) break;
        }
    }
    ...
    fn negotiateV2(self: *DaemonClient) bool {
        ...
        self.writeFrame(hello) catch return false;
        const response = self.receiveFrame() catch return false;
        defer self.allocator.free(response);
        if (!Wire.looksLikeV2(response)) return false;
        if (std.mem.indexOf(u8, response, "\"kind\":\"hello\"") == null) return false;
        self.selected_version = if (std.mem.indexOf(u8, response, "\"selectedVersion\":2") != null) 2 else 1;
        if (self.selected_version != 2) return false;
        return true;
    }
    ...
    fn receiveFrame(self: *DaemonClient) ![]u8 {
        var header: [4]u8 = undefined;
        try readAll(self.pipe, &header);
        const length = try Wire.decodedLength(header, self.mode);
        const bytes = try self.allocator.alloc(u8, length);
        errdefer self.allocator.free(bytes);
        try readAll(self.pipe, bytes);
        return bytes;
    }
...
fn writeAll(handle: c.HANDLE, bytes: []const u8) !void {
    ...
    while (offset < bytes.len) {
        ...
        if (c.WriteFile(handle, bytes[offset..].ptr, amount, &written, null) == 0 or written == 0) {
...
fn readAll(handle: c.HANDLE, bytes: []u8) !void {
    var offset: usize = 0;
    while (offset < bytes.len) {
        var read: c.DWORD = 0;
        const amount: c.DWORD = @intCast(@min(bytes.len - offset, std.math.maxInt(c.DWORD)));
        if (c.ReadFile(handle, bytes[offset..].ptr, amount, &read, null) == 0 or read == 0) {
