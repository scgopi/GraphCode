...
$mailroomLink = Join-Path $sources "MailroomKit"
$mailroomTarget = Join-Path $repo "MailroomKit\Sources"

New-Item -ItemType Directory -Force -Path $sources | Out-Null
if (Test-Path $link) {
  Remove-Item $link
}
if (Test-Path $mailroomLink) {
  Remove-Item $mailroomLink
}
New-Item -ItemType Junction -Path $link -Target $target | Out-Null
New-Item -ItemType Junction -Path $mailroomLink -Target $mailroomTarget | Out-Null
