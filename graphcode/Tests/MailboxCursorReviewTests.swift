...
struct MailboxCursorReviewTests {
  ...
  /// `prunedUnread` is computed only for a reader whose cursor is set. A loop that was
  /// created and has never run `mail inbox` has `lastMailroomRead == nil`, so mail that
  /// landed during its life and was pruned before it read is reported as nothing lost —
  /// the exact silence the counter exists to break.
  @Test
  func aReaderThatNeverReadIsStillToldWhatItLost() async throws {
    let store = makeStore()
    let reader = await makeReader(in: store)
    #expect(await store.graph.nodes[id: reader]?.lastMailroomRead == nil)
    for index in 1...(Mailroom.maxNotices + 60) {
      await store.handle(.mailroomPost(text: "note \(index)", topic: nil, from: nil))
    }
