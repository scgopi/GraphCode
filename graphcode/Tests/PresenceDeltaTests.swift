...
struct PresenceDeltaTests {
  ...
  /// Only a connection that announced the capability gets the delta; every other gets
  /// the whole snapshot for the same tick — so an app that predates deltas is never
  /// sent a frame it cannot read. An announcement that arrives after the join counts.
  @Test
  func aTickReachesEachConnectionInTheShapeItAnnounced() async throws {
    let readings = Readings()
    var graph = LoopGraph(project: Self.project)
    graph.nodes.append(node("Moving"))
    let store = GraphStore(
      graph: graph, onEnsureSession: { _, _ in },
      onReadPresence: { node, _ in await readings.read(node) })
    var modern: [Int32] = [0, 0]
    var legacy: [Int32] = [0, 0]
    #expect(socketpair(AF_UNIX, SOCK_STREAM, 0, &modern) == 0)
    #expect(socketpair(AF_UNIX, SOCK_STREAM, 0, &legacy) == 0)
    defer {
      OutboundChannels.close(modern[0])
      OutboundChannels.close(legacy[0])
      close(modern[1])
      close(legacy[1])
    }
    let modernID = UUID()
    await store.addConnection(id: modernID, fileDescriptor: modern[0])
    await store.addConnection(id: UUID(), fileDescriptor: legacy[0])
    _ = try await nextEvent(from: modern[1])
    _ = try await nextEvent(from: legacy[1])
    // The app's announcement lands after its join; the registry forwards it.
    await store.setCapabilities([ClientCapability.nodesChanged.rawValue], for: modernID)

    await readings.set("Moving", .busy)
    await store.pollPresence()

    guard case .nodesChanged(_, let revision, let moved) = try await nextEvent(from: modern[1])
    else {
      Issue.record("the announcing connection should get the delta")
      return
    }
    #expect(moved.map(\.title) == ["Moving"])
    guard case .graphChanged(let snapshot) = try await nextEvent(from: legacy[1]) else {
      Issue.record("a connection that announced nothing should get the snapshot")
      return
    }
    #expect(snapshot.revision == revision)
    #expect(snapshot.nodes[0].presence?.presence == .busy)
    #expect(snapshot.mailroom.isEmpty)
  }

  /// The invariant lives in the type, not in memory: every event either predates
  /// capabilities (sent to everyone, as always) or names the capability that gates it.
  /// A new case does not compile until its author has decided which, and `deliver`
  /// enforces the answer for every call site.
  @Test
  func everyEventSaysWhetherAnOlderClientMayReceiveIt() {
    let legacy: [DaemonEvent] = [
      .recentProjectsListed([]), .graphChanged(LoopGraph(project: Self.project)),
      .errorOccurred("x"),
      .mailbox(
        projectPath: "/p",
        mailbox: Mailbox(posts: [], bodiesTrimmed: false, digest: MailroomDigest(of: []))),
    ]
    #expect(legacy.allSatisfy { $0.requiredCapability == nil })
    #expect(
      DaemonEvent.nodesChanged(projectPath: "/p", revision: 1, nodes: []).requiredCapability
        == .nodesChanged)
  }
