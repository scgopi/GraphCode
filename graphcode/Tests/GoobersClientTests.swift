...
struct GoobersClientTests {
        ...
        return (
          Data(
            """
            {
              ...
              "ready": true,
              "healthy": true,
              "instance": {"name": "demo", "environment": "dev"}
            }
            """
            .utf8), 200
        )
      case "/api/v1/runs":
        #expect(url.url.query == "limit=12")
        return (
          Data(
            """
            {
              "runs": [{
                ...
                "workflowVersion": 1,
                "gaggle": "demo",
                "trigger": {"kind": "schedule", "ref": "@hourly"},
                "phase": "running",
                "terminal": false,
                "currentStage": "review",
                "startedAt": "2026-09-03T18:00:00Z",
                "durationMillis": 1000,
                "lastActivityAt": "2026-09-03T18:00:01Z",
                "stale": false,
                "repassCount": 1,
                "retryCount": 0,
                "noWork": false
  ...
  func boundsTheRunLimitToTheGoobersContract() async throws {
    ...
    defer { try? FileManager.default.removeItem(at: root) }
    let client = GoobersClient(instanceRoot: root) { url in
      #expect(url.url.query == "limit=200")
  ...
  @Test
  func triggersAWorkflowWithAStableDeliveryIdentity() async throws {
    let root = try instance()
    defer { try? FileManager.default.removeItem(at: root) }
    let client = GoobersClient(instanceRoot: root) { request in
      #expect(request.url.path == "/api/v1/triggers")
      #expect(request.method == "POST")
      let body = try #require(request.body)
      let json = try #require(JSONSerialization.jsonObject(with: body) as? [String: String])
      #expect(json["gaggle"] == "graphcode")
      #expect(json["workflow"] == "demo")
      #expect(json["requestId"] == "delivery-1")
      return (Data(#"{"runId":"run-42","duplicate":false}"#.utf8), 200)
    }

    let response = try await client.trigger(
      gaggle: "graphcode", workflow: "demo", requestID: "delivery-1")
    #expect(response.runId == "run-42")
